...
def helper(dis):
    try:
        ...
        pre = pre[0].tolist() if pre.size > 0 else ["No precautions available."]

        med = medications[medications['Disease'] == dis]['Medication'].values
        med = med.tolist() if med.size > 0 else ["No medications available."]

        die = diets[diets['Disease'] == dis]['Diet'].values
        die = die.tolist() if die.size > 0 else ["No dietary recommendations available."]

        wrkout = workout[workout['disease'] == dis]['workout'].values
        wrkout = wrkout.tolist() if wrkout.size > 0 else ["No workout suggestions available."]
...
def get_predicted_value(patient_symptoms):
    try:
        ...
        return f"Error predicting disease: {str(e)}"
